...
FRIEND_LIST_PATH = os.path.join(
    THIS_FOLDER, "userfriends.json"
)  # Create path of friendlist json. (name can be changed)
...
try:
    ...
    BOT_DATA.read_json(
        FRIEND_LIST_PATH
    )  # Should not raise any exceptions but here just in case.
    ...
    while not BOT.is_closed():
        ...
        print(
            "{}\nBot by Raz Kissos, helper and useful functions.\nFriend list json file path: {}\n********************\n".format(
                DATETIME_OBJ.today(), FRIEND_LIST_PATH
            )
        )
...
async def console():
    ...
    commands = [
        ...
        ("exit", "closes the bot"),
        ("guilds", "prints the current guilds the bot is on"),
    ...
    while True:
        try:
            ...
            if command == commands[0][0]:  # help console command.
                ...
                print("********************")
                print(
                    "~BOT BY MARSE~\n-Prefix: {}\n-Token: {}\n-Friend List Path: {}".format(
                        BOT_DATA.BOT_PREFIX, BOT_DATA.TOKEN, FRIEND_LIST_PATH
                    )
                )
                print("********************")
        except Exception as e:
...
async def ban(ctx, member: discord.Member, *, reason: str):
    if member in ctx.guild.members:
        ...
        await ctx.channel.send(
            f'Banned the user {member.mention} for reason: "{reason}"'
        )
    else:
...
@BOT.command(
    ...
    aliases=[
        ...
        "8-ball",
        "8B",
    ],  # returns a random str for a deciding factor.
)
...
async def Weather(ctx, coords: str):
    ...
    url = "https://fcc-weather-api.glitch.me/api/current?lat={}&lon={}".format(
        lat_lon[0], lat_lon[1]
    )
    response = json.loads(requests.get(url).content)
    weather_type = response["weather"][0]["main"] + " ({})".format(
        response["weather"][0]["description"]
    )
    temp = str(response["main"]["temp"]) + "°"
    await ctx.send(
        "The weather in {} today is {} and the temperature is {}.".format(
            coords, weather_type, temp
        )
    )
...
async def looking_to_play(ctx, game: str):
    ...
    if str(ctx.author.id) not in data:
        await ctx.channel.send(
            "Too bad! seems you are lonely as fuck and do not have any friends!"
        )
    else:
        ...
        for friend_id in friends:
            for user in all_usr_list:
                if friend_id == str(user.id):
                    ...
                    await friend_obj.send(
                        "Hi {}, {} wants to play {} with you!".format(
                            friend_obj.mention, ctx.author.name, game
                        )
                    )

        await ctx.channel.send(
            "{} :\n{} is in need of your assistace! join him in his mighty {} game!".format(
                " ".join(list_of_mentions), ctx.author.mention, game
            )
        )
...
async def add_friend(ctx):
    ...
    if len(ctx.message.mentions) > 0:
        ...
        if str(ctx.author.id) in json_data:
            for mention in ctx.message.mentions:
                if mention == ctx.message.author:
                    await ctx.send(
                        "You cannot add yourself as a friend! Go find some real ones..."
                    )
                else:
                    if str(mention.id) not in json_data[str(ctx.author.id)]:
                        ...
                        await ctx.channel.send(
                            f"{mention.mention} already in your friend list!"
                        )
        else:
            friend_list = list()
            for mention in ctx.message.mentions:
        ...
        await ctx.channel.send(
            "Parameters invalid! check out {}help".format(BOT_DATA.BOT_PREFIX)
        )
    ...
    if len(ctx.message.mentions) > 0:
        ...
        if str(ctx.author.id) in data:
            for mention in ctx.message.mentions:
                if mention == ctx.message.author:
                    await ctx.send(
                        "Do you really hate yourself that much that you wish to unfriend yourself? pathetic."
                    )
                else:
                    if str(mention.id) not in data[str(ctx.author.id)]:
                        await ctx.channel.send(
                            f"{mention.mention} not in your friend list!"
                        )
                    else:
                        data[str(ctx.author.id)].remove(str(mention.id))
                        await ctx.channel.send(
                            f"{mention.mention} removed successfully!"
                        )
        else:
            await ctx.channel.send(
                "Too bad! seems you are lonely as fuck and do not have any friends!"
